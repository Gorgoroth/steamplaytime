...
A lot of this isn't currently being used in the steamplaytime app.
"""
...
class SteamUsers(SteamAPI):
	...
	called with Users.is_private(steamid)
		...
		if self.visibility:
			...
			for game in json_data:
				...
				if 'hours_forever' in game:
					...
					hours_played = 0.0
				if appid not in specialcases.outlawed_games:
	...
	def get_items(self, game):
		...
		if game == "tf2" or game == "TF2":
			...
			raise BadGameException("Invalid game.")

		url = "http://api.steampowered.com/IEconItems_%s/GetPlayerItems/v0001/?key=%s&SteamID=%s" % (game_id, self.api_key, self.steam_id)
		json_data = self._get_json(url)
	...
	def get_gifts(self):
		url = "http://steamcommunity.com/profiles/%s/inventory/json/753/1/" % self.steam_id
		json_data = self._get_json(url)
		gifts_list = []
